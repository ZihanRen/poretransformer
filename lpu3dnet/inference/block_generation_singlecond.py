...
class Block_generator_stochastic:
    ...
    def generate_token(self,token_input,cond_input,top_k,temperature):
        ...
        cond_input = cond_input[:,:,0].unsqueeze(2)
...
class Block_generator_compare:
    def __init__(self,
        ...
        self.epoch_transformer = epoch_transformer
        self.img = img
        print("The shape of comparing image is {}".format(img.shape))
    ...
    def patchify_img(self):
        ...
        assert self.img.shape[0] == 64*self.volume_dimension

        # segment images into 64^3 cubic voxels in 2*2*2 spatial grid
        for i in range(self.volume_dimension):
            for j in range(self.volume_dimension):
                for k in range(self.volume_dimension):
                    subset_image = self.img[i*64:(i+1)*64, j*64:(j+1)*64, k*64:(k+1)*64]
                    key_idx = (i, j, k)
                    self.ds_spatial[key_idx]['img_original'] = subset_image
                    porosity = ps.metrics.porosity(subset_image)
                    if porosity < 5e-2: # if <= 0.05, porosity = 0.05
                        porosity = 0.05 # TODO: tune this parameter
